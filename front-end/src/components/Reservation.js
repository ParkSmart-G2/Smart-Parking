...
const ReservationForm = ({ userData }) => {
  ...
  const [numberOfDays, setNumberOfDays] = useState("");
  const [price, setPrice] = useState(0);
  console.log(userData, "from reservation");
  useEffect(() => {
    calculatePrice();
  }, [numberOfHours, numberOfDays]);
  ...
  const handleReservation = async () => {
    try {
      const response = await axios.post(
        "http://localhost:4444/api/reservation/create",
  ...
  if (userData.status === "reserved") {
    return (
      <div>
        <p>
          You have reserved at: {userData.startingDate} until{" "}
          {userData.startingDate}{" "}
        </p>
        <button onClick={() => console.log("Update reservation")}>
          Update
        </button>
        <button onClick={() => console.log("Cancel reservation")}>
          Cancel
        </button>
        <button onClick={() => console.log("Update reservation")}>
          I Arrived
